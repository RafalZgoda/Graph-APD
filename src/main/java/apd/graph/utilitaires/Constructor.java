...
public class Constructor {
	public static void main(String[] args) {
		...
		try {
			...
			String line;
			/* Boucle sur chaque ligne qui regarde le premier caract�re (si le premier char est c, ignore la ligne */
			while ((line = reader.readLine()) != null) {
				if (line.length() >= 1) {
					char first = line.charAt(0);
					if (first == 'c') {
						System.out.println("Commentaire");
						/* Parcourt les caract�res suivants pour recup�r�s les deux noeuds*/
					} else if (first == 'e') {
						int i = 1;
						boolean token = true;
						String node1 = "", node2 = "";
						...
						/* Fin du parcours de la ligne, cr�ation des deux sommets (si il n'existe pas) et de l'arrete */ 
